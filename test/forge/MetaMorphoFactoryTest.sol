...
contract MetaMorphoFactoryTest is BaseTest {
    ...
    function testDeploy(address morpho, uint256 initialTimelock, string memory name, string memory symbol, bytes32 salt)
        public
    {
        ...
        bytes32 initCodeHash = hashInitCode(
            type(MetaMorpho).creationCode, abi.encode(morpho, initialTimelock, address(loanToken), name, symbol)
        );
        address expectedAddress = computeCreate2Address(salt, initCodeHash, address(factory));

        vm.expectEmit(address(factory));
        emit EventsLib.Deployed(
            expectedAddress, address(this), morpho, initialTimelock, address(loanToken), name, symbol, salt
        );

        MetaMorpho metaMorpho = factory.deploy(morpho, initialTimelock, address(loanToken), name, symbol, salt);
