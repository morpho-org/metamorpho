...
contract VaultTest is BaseTest {
    ...
    function testMintWithCaps(uint128 cap, uint256 assets) public {
        cap = uint128(bound(cap, MIN_TEST_ASSETS, MAX_TEST_ASSETS));
        assets = bound(assets, MIN_TEST_ASSETS / 3, 3 * cap);
        ...
        assertEq(vault.balanceOf(ONBEHALF), shares, "balance");
        _assertBalances(cap, amount);
    }

    function testDepositWithCaps(uint128 cap, uint256 amount) public {
        cap = uint128(bound(cap, MIN_TEST_ASSETS, MAX_TEST_ASSETS));
        amount = bound(amount, MIN_TEST_ASSETS / 3, 3 * cap);
    ...
    function testShouldNotMintMoreThanCaps(uint128 cap, uint256 amount) public {
        cap = uint128(bound(cap, MIN_TEST_ASSETS, MAX_TEST_ASSETS / 3));
        amount = bound(amount, 3 * cap + 1, MAX_TEST_ASSETS);
        ...
        public
    {
        cap = uint128(bound(cap, MIN_TEST_ASSETS, MAX_TEST_ASSETS / 3));
        alreadyDeposited = bound(alreadyDeposited, 3 * cap / 2, 3 * cap);
        amount = bound(amount, 3 * cap - alreadyDeposited + 1, MAX_TEST_ASSETS);
