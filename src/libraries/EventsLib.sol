...
library EventsLib {
    ...
    /// @notice Emitted `rewardsDistibutor` is set to `newRewardsDistributor`.
    event SetRewardsDistributor(address indexed newRewardsDistributor);
    ...
    /// @notice Emitted when an `amount` of `token` is transferred to the `rewardsDistributor` by `caller`.
