...
contract MetaMorpho is ERC4626, ERC20Permit, Ownable2Step, Multicall, IMetaMorpho {
        ...
        for (uint256 i; i < currLength; ++i) {
            if (!seen[i]) {
                ...
                try this.expectedSupplyAssets(_marketParams(id)) returns (uint256 supplyAssets) {
                    // Assumes that a market with zero supply assets (but possibly non-zero supply shares) is dust.
    ...
    function _simulateWithdrawMorpho(uint256 assets) internal view returns (uint256 remaining) {
        ...
        for (uint256 i; i < withdrawQueue.length; ++i) {
            ...
            // Withdrawing from Morpho cannot fail in `_withdrawMorpho` because:
            // 1. oracle.price() is never called (the vault doesn't borrow);
            // 2. virtually accruing interest didn't fail in `_withdrawable`;
            // 3. `_withdrawable` caps to the liquidity available on Morpho.
